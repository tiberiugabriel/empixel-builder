...
export const BASE_DEFAULTS: Record<string, any> = {
  ...
  styleHover: {},
  styleDark: {},
  // F4.5 — hover-on-dark-mode override. Empty by default; populated by
  // authors via the Style tab when they want a different hover treatment
  // on dark theme. Falls back to `styleHover` on dark when empty (the
  // dark-hover CSS rule simply doesn't emit), so existing layouts
  // render byte-identical to pre-F4.5 except for the `!important` drop.
  styleHoverDark: {},
  styleBreakpoints: {},
  styleHoverBreakpoints: {},
  // F4.5 — per-breakpoint hover-on-dark-mode override. Same shape as
  // `styleHoverBreakpoints`: `{ [bpId]: { _px, ...CSSProps } }`. Empty
  // map default; only populated when the author edits a hover value
  // on a non-desktop breakpoint while ThemeStyleToggle is set to Dark.
  styleBreakpointsHoverDark: {},
  advanced: { ...EMPTY_ADVANCED_DEFAULTS },
};
